...
fit.model <- function(dataset, model.type, resp.variable, expl.variables, model.parameters) {
  ...
  return(list(
    coefficients = summary(model, se = "iid")$coefficients[, 1], significance = summary(model, se = "iid")$coefficients[, 4],
...
fit.model.with.breakpoints <-
  function(dataset, model.type, resp.variable, expl.variables.without.segm.variable, segm.variable, breakpoints, model.parameters, angle.param = 0) {
    ...
    return(list(
      dataset = dataset, coefficients = fit$coefficients, significance = fit$significance,
...
breakpoint.models.fixed.interval <- function(dataset, model.type,
  ...
  next.breakpoints <- add.extra.coordinate(
    current.set = fixed.breakpoints, number.of.intervals = number.of.intervals,
  ...
  while (!identical(next.breakpoints, "max permitted")) {
    ...
    next.breakpoints <-
      next.set.to.test.change.last.coordinate.only(
        current.set = next.breakpoints,
        number.of.intervals = number.of.intervals, min.distance = min.distance
      )$next.set
  }
  ...
  for (b in list.of.breakpoints)
  {
    ...
    transf.breakpoints <-
      transform.breakpoint(
        start.point = start.point, breakpoint = b,
        interval.length = interval.length, time.unit = time.unit
      )$transf.breakpoint
    # fit a model corresponding to the current breakpoints
    temp.fit <- fit.model.with.breakpoints(
      dataset = dataset, model.type = model.type, resp.variable = resp.variable,
    ...
    list.of.models[[paste("breakpoints=", b, collapse = ",")]] <- list(
      number.of.breakpoints = length(b),
  ...
  return(list(
    list.of.breakpoints = list.of.breakpoints,
...
search.for.new.breakpoint.fixed.interval <- function(dataset, model.type,
  ...
  alpha <- breakpoint.models.fixed.interval(
    dataset = dataset, model.type = model.type,
...
sequential.search.for.new.breakpoint.whole.interval <- function(dataset, model.type,
  ...
  transf.temp.end.point <-
    transform.breakpoint(
      start.point = as.POSIXct(start.point), breakpoint = temp.end.point,
  ...
  while (transf.temp.end.point <= end.point) {
    alpha <- search.for.new.breakpoint.fixed.interval(
    ...
    transf.temp.end.point <-
      transform.breakpoint(
        start.point = start.point, breakpoint = temp.end.point,
        interval.length = interval.length, time.unit = time.unit
      )$transf.breakpoint
    temp.dataset <- eval(parse(text = paste("subset(dataset, ", segm.variable, " <transf.temp.end.point)")))
  }
  ...
  if (found.new.breakpoint == FALSE) {
    ...
    transf.breakpoints <-
      transform.breakpoint(
        start.point = start.point, breakpoint = fixed.breakpoints,
        interval.length = interval.length, time.unit = time.unit
      )$transf.breakpoint
    new.best.model.dataset <- dataset
    temp.fit <- fit.model.with.breakpoints(
      dataset = new.best.model.dataset,
      model.type = model.type, resp.variable = resp.variable,
      expl.variables.without.segm.variable = expl.variables.without.segm.variable, segm.variable = segm.variable,
      breakpoints = transf.breakpoints, model.parameters = model.parameters, angle.param = angle.param
    )
    new.best.model <- list(
      number.of.breakpoints = length(fixed.breakpoints),
  ...
  return(list(
    found.new.breakpoint = found.new.breakpoint, new.end.point = new.end.point,
...
sequential.grid.search.save.models <- function(dataset, model.type,
  ...
  while (found.new.breakpoint) {
    alpha <- sequential.search.for.new.breakpoint.whole.interval(
      dataset = dataset,
      model.type = model.type, resp.variable = resp.variable,
      expl.variables.without.segm.variable = expl.variables.without.segm.variable,
      segm.variable = segm.variable,
    ...
    models <- append(models, list(list(
      best.model = alpha$new.best.model, good.breakpoint.models = alpha$good.breakpoint.models,
      start.point = start.point,
      end.point = transform.breakpoint(
        start.point = start.point,
...
summary.of.output <- function(list.of.models, dataset, model.type, resp.variable, expl.variables.without.segm.variable, segm.variable, model.parameters, angle.param = 0) {
  ...
  temp <- fit.model.with.breakpoints(
    dataset = dataset, model.type = model.type, resp.variable = resp.variable,
    expl.variables.without.segm.variable = expl.variables.without.segm.variable, segm.variable = segm.variable,
...
plot.fit <- function(dataset, model, summary.dataset,
  ...
  if (!identical(factor.expl.variables, "") & !identical(factor.expl.variables, character())) {
    p <- p + geom_line(aes(y = untransf.predict, colour = factors, group = factors), size = 0.5)
  } else {
...
master.isr <- function(dataset,
  ...
  isr.models <- sequential.grid.search.save.models(
    dataset = new.dataset, model.type = model.type, resp.variable = new.resp.variable,
    expl.variables.without.segm.variable = expl.variables.without.segm.variable,
    segm.variable = segm.variable,
    model.parameters = model.parameters, angle.param = angle.param,
    start.point = start.point, end.point = end.point,
  ...
  output <- summary.of.output(
    dataset = new.dataset, list.of.models = isr.models$models,
  ...
  require(ggplot2)
  plot.fit <- plot.fit(
    dataset = output$dataset, model = output$best.model, summary.dataset = output$summary.dataset,
